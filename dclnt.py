...
def get_all_words_in_path(path):
    ...
    return flat([split_snake_case_name_to_words(
...
def get_top_verbs_in_path(path, top_size=10):
    ...
    fncs = [f for f in flat(
        [[node.name.lower() for node in ast.walk(t) if isinstance(
            node, ast.FunctionDef)] for t in trees]) if not (
                f.startswith('__') and f.endswith('__'))]

    print('functions extracted')
    verbs = flat([get_verbs_from_function_name(
        function_name) for function_name in fncs])
...
def get_top_functions_names_in_path(path, top_size=10):
    ...
    nms = [f for f in flat(
        [[node.name.lower() for node in ast.walk(t) if isinstance(
            node, ast.FunctionDef)] for t in t]) if not (
            f.startswith('__') and f.endswith('__'))]
